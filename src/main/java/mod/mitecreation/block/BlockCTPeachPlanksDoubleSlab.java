...
public class BlockCTPeachPlanksDoubleSlab extends BlockDoubleSlab {
    ...
    public BlockCTPeachPlanksDoubleSlab(int id, BlockSlab single_slab) {
        super(id, single_slab);
        if (single_slab != this.getSingleSlab()) {
            Minecraft.setErrorMessage("BlockCTPeachPlanksDoubleSlab: single slab mismatch");
        }
    ...
    private BlockSlab getSingleSlab() {
        if (this.single_slab == null) {
            this.single_slab = (BlockSlab)Block.getBlock(CTRegistryInit.peachPlanksSingleSlab.blockID);
        }
    ...
    public int dropBlockAsEntityItem(BlockBreakInfo info) {
        if (info.wasExploded()) {
            ...
            if (this.blockMaterial == Material.wood) {
                model_block = CTRegistryInit.peachPlanks;
                info.setMetadata(0);
            } else {
